...
function LeadDetails() {
  ...
  const [attachmentNameToDelete, setAttachmentNameToDelete] =
    useState<string>('');
    ...
    fileInput.addEventListener('change', async (event: any) => {
      ...
      if (files && files[0]) {
        ...
        try {
          ...
          const result = await uploadAndAttachFileToLead(
            leadId as string,
            file,
            headers
          );
  ...
  const handleConvertClick = () => {
    ...
    if (leadData && leadData?.lead_obj?.status !== 'qualified') {
      setAlertMessage('Lead must be in Qualified status to convert');
      setAlertType('error');
      setAlertOpen(true);
      return;
    }
  ...
  const handleConvertConfirm = () => {
    ...
    // navigate to Lead to Opportunity page
    navigate('/app/opportunities/add-opportunity-from-lead', {
      state: {
        leadData: leadData,
      },
    });
  ...
  const confirmAttachmentDelete = () => {
    ...
    fetchData(
      `/${LeadUrl}/attachment/${attachmentToDelete}/`,
      'DELETE',
      null as any,
      Header
    )
  ...
  return (
    <Box sx={{ mt: '60px' }}>
      ...
      <DeleteModal
        ...
        modalDialog={`Are you sure you want to delete${
          attachmentNameToDelete
            ? ` "${attachmentNameToDelete}"`
            : ' this attachment'
        } from ${leadData?.lead_obj?.contact?.first_name || ''} ${
          leadData?.lead_obj?.contact?.last_name || 'Doe'
        }'s lead?`}
      />
      ...
      <DialogModal
        ...
        modalDialog={`Are you sure you want to convert ${
          leadData?.lead_obj?.lead_title || ''
        } 's lead to oppurtunity?`}
        confirmText="Convert"
        cancelText="Cancel"
      />
                  ...
                  >
                    <Box>
                      <Typography variant="h4" component="h1" sx={{ mb: 2 }}>
                        {capitalizeFirstLetter(leadData?.lead_obj?.lead_title)}{' '}
                        Lead
                      </Typography>
                      <Box
                        sx={{
                      ...
                      >
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <StatusLabel
                            status={leadData?.lead_obj?.status || 'Qualified'}
                          />
                        </Box>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <FaEnvelope
                    ...
                    >
                      <Button
                        variant="contained"
                        color={leadData?.converted ? 'success' : 'primary'}
                        sx={{
                          borderRadius: '4px',
                          width: '100%',
                          textTransform: 'capitalize',
                        ...
                        disabled={leadData?.converted}
                      >
                        {leadData?.converted ? 'Converted' : 'Convert'}
                      </Button>
                ...
                >
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                    <Box sx={{ flex: '0 0 33%', mb: 2 }}>
                      ...
                      <Typography variant="body1">
                        {leadData?.lead_obj?.link || 'https://link.com'}
                      </Typography>
                    </Box>
                    ...
                    <Box sx={{ flex: '0 0 33%', mb: 2 }}>
                      ...
                      <Typography variant="body1">
                        {capitalizeFirstLetter(
                          leadData?.lead_obj?.assigned_to?.user_details
                            ?.first_name
                        ) || 'Johny'}{' '}
                        {capitalizeFirstLetter(
                          leadData?.lead_obj?.assigned_to?.user_details
                    ...
                    <Box sx={{ flex: '0 0 33%', mb: 2 }}>
                      ...
                      {leadData?.lead_obj?.lead_attachment &&
                      leadData.lead_obj.lead_attachment.length > 0 ? (
                        <Box
                          sx={{
                            display: 'flex',
                            flexDirection: 'column',
                        ...
                        >
                          {leadData.lead_obj.lead_attachment.map(
                            (attachment, index) => {
