...
function EditContact() {
  ...
  // Handle scroll to top and focus when component mounts
  useEffect(() => {
    window.scrollTo(0, 0);
  ...
  // Populate form data when contact details are available
  useEffect(() => {
    if (state?.value) {
      setFormData({
        salutation: state.value.salutation || '',
        first_name: state.value.first_name || '',
        last_name: state.value.last_name || '',
        primary_email: state.value.primary_email || '',
        mobile_number: state.value.mobile_number || '',
  ...
  // Initialize Quill editor with description content
  useEffect(() => {
    ...
    try {
      ...
      if (res.success || !res.error) {
        ...
        if (res.details && typeof res.details === 'object') {
          ...
          // Also set general error message if provided
          if (res.message) {
  ...
  const isFormDirty = () => {
    ...
    return (
      formData.salutation !== (state.value.salutation || '') ||
      formData.first_name !== (state.value.first_name || '') ||
      formData.last_name !== (state.value.last_name || '') ||
      formData.primary_email !== (state.value.primary_email || '') ||
      formData.mobile_number !== (state.value.mobile_number || '') ||
      formData.company !== (state.value.company || '') ||
  ...
  // Reset Quill editor to initial state
  function resetQuillToInitialState(
    ...
    if (quill && initialContentRef.current !== null) {
      ...
      setFormData((prev) => ({
        ...prev,
        description: initialContentRef.current || '',
      }));
    }
  }
  ...
  return (
    <Box sx={{ mt: '60px' }} ref={pageContainerRef}>
      {/* Custom AppBar with navigation and form controls */}
      <CustomAppBar
        module={module}
        crntPage={crntPage}
        backBtn={backBtn}
      ...
      <Box sx={{ mt: '120px' }}>
        <form onSubmit={handleSubmit}>
          <div style={{ padding: '10px' }}>
            ...
            {/* Description section with Quill rich text editor */}
            <div className="leadContainer">
