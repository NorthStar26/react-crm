...
const ALLOWED_FILE_EXTENSIONS = [
  ...
  'zip', 'rar', '7z', 'tar', 'gz',
  // Design files
  'psd', 'ai', 'eps', 'ttf', 'otf', 'woff', 'woff2',
  // Images
  'jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'tiff', 'ico', 'heic', 'svg', 'avif', 'jfif'
];
...
export const uploadFileToCloudinary = async (file: File) => {
  ...
  if (!uploadPreset) {
    return {
      success: false,
      error: 'Cloudinary upload preset is not defined in environment variables.',
    };
  }
  ...
  const extension = file.name.split('.').pop()?.toLowerCase() || '';
  const documentTypes = ['pdf', 'doc', 'docx', 'txt', 'rtf', 'ppt', 'pptx', 'xls', 'xlsx', 'csv'];
 ...
 */
export const attachFileToLead = async (leadId: string, fileUrl: string, fileName: string, fileType: string, headers: any) => {
  try {
 ...
 */
export const uploadAndAttachFileToLead = async (leadId: string, file: File, headers: any) => {
  ...
  const attachResult = await attachFileToLead(leadId, apiUrl, fileName, fileType, headers);
